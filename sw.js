/* Terskel PWA cache. User data remains in localStorage. */
const CACHE='terskel-static-v2.0.0';
...
self.addEventListener('activate',event=>{
  event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith('terskel-static-')&&k!==CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim()));
});
